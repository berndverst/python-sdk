...
from google.protobuf import empty_pb2, any_pb2
...
from dapr.proto.runtime.v1 import appcallback_pb2
...
class _CallbackServicer(appcallback_service_v1.AppCallbackServicer):
    ...
    def OnInvoke(self, request: InvokeRequest,
    ...
    def OnTopicEvent(
            self, request: appcallback_pb2.TopicEventRequest,
            context: grpc.ServicerContext) -> appcallback_v1.TopicEventResponse:
        ...
        for k, v in request.attributes.items():
            field = v.WhichOneof(oneof_name='attr')
            if field:
                attributes[k] = getattr(v, field)
            else:
                attributes[k] = None
        event.SetExtensions(attributes)
    ...
    def OnBindingEvent(
            self, request: appcallback_pb2.BindingEventRequest,
