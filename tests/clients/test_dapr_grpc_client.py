...
class DaprGrpcClientTests(asynctest.TestCase):
    ...
    async def test_http_extension(self):
        ...
        ext = await dapr._get_http_extension('POST', qs)
    ...
    async def test_get_configuration(self):
        ...
        resp = await dapr.get_configuration(store_name="configurationstore", keys=key)
        self.assertEqual(resp.items[0].key, key)
        ...
        resp = await dapr.get_configuration(store_name="configurationstore", keys="NotValidKey")
