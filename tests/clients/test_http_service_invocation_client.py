...
class DaprInvocationHttpClientTests(asynctest.TestCase):
    def setUp(self):
    ...
    async def test_basic_invoke(self):
        ...
        response = await self.client.invoke_method(self.app_id, self.method_name, "")

        self.assertEqual(b"STRING_BODY", response.data)
        self.assertEqual(self.invoke_url, self.server.request_path())

    async def test_invoke_PUT_with_body(self):
        self.server.set_response(b"STRING_BODY")

        response = await self.client.invoke_method(self.app_id, self.method_name, b"FOO",
                                                   http_verb='PUT')
    ...
    async def test_invoke_PUT_with_bytes_body(self):
        self.server.set_response(b"STRING_BODY")
    ...
    async def test_invoke_GET_with_query_params(self):
        self.server.set_response(b"STRING_BODY")
    ...
    async def test_invoke_GET_with_duplicate_query_params(self):
        self.server.set_response(b"STRING_BODY")
        query_params = (('key1', 'value1'),
                        ('key1', 'value2'))
    ...
    async def test_invoke_PUT_with_content_type(self):
        self.server.set_response(b"STRING_BODY")
    ...
    async def test_invoke_method_metadata(self):
    ...
    async def test_invoke_method_protobuf_response_with_suffix(self):
        self.server.set_response(b"\x0a\x04resp")
    ...
    async def test_invoke_method_protobuf_response_case_insensitive(self):
        self.server.set_response(b"\x0a\x04resp")
        self.server.reply_header('Content-Type', 'apPlicaTion/x-protobuf; gzip')
    ...
    async def test_invoke_method_error_returned(self):
        ...
        with self.assertRaises(DaprInternalError) as ctx:
            await self.client.invoke_method(
                self.app_id,
                self.method_name,
                http_verb='PUT',
                data='FOO',
    ...
    async def test_invoke_method_non_dapr_error(self):
    ...
    async def test_generic_client_unknown_protocol(self):
    ...
    async def test_invoke_method_with_api_token(self):
        self.server.set_response(b"FOO")
    ...
    async def test_invoke_method_with_tracer(self):
    ...
    async def test_timeout_exception_thrown_when_timeout_reached(self):
        new_client = DaprClient(http_timeout_seconds=1)
        self.server.set_server_delay(1.5)
        with self.assertRaises(TimeoutError):
            await new_client.invoke_method(self.app_id, self.method_name, "")

    async def test_global_timeout_setting_is_honored(self):
